...
docker rm project_app project_database project_phpmyadmin -f
...
while ! docker exec project_database mysqladmin --user=root --password=secret --host "127.0.0.1" ping --silent &> /dev/null ; do
